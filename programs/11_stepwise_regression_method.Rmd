...
```{r}
# File names updated due to higher number of rows with updated data
match5 = read.csv(file.path(match_dir, 'fiveDay_LS-Secchi_matchups_n361.csv'))
match71 = read.csv(file.path(match_dir, 'multiDay_LS-Secchi_matchups_n362.csv'))

stack = read.csv(file.path(rs_dir, 'yojoa_corr_rrs_met_scaled_v2026-03-24.csv'))
secchi = read.csv(file.path(is_dir, 'Secchi_completedataset.csv')) %>% 
  #bind with updated data
  bind_rows(read.csv(file.path(is_dir, 'Secchi_updates.csv')) %>% mutate(secchi = as.character(secchi))) %>% 
...
```{r}
# there are NAs in test data
test_match71<- test_match71 %>% drop_na()
...
```{r}
test_match5 <- test_match5 %>% drop_na()
