...
author: "B Steele w/Edits from Matt Ross and Caitlin Mothes"
date: "`r Sys.Date()`"
...
hypertune_xgboost = function(train, test, grid){
  ...
  cb.log <- xgb.cb.evaluation.log()
  
  xgb.naive <- xgb.train(params = params, data = train, nrounds = 1000, 
                         evals = list(train = train, val = test), 
                         verbose = 0,
                         early_stopping_rounds = 20,
                         callbacks = list(cb.log))
  
  eval_log <- get("evaluation_log", envir = cb.log$env)
  val_scores <- sapply(eval_log, function(x) x$metrics["val-rmse"])
  best_score <- min(val_scores)
  best_iter <- which.min(val_scores)
  
  summary <- grid %>% mutate(val_loss = best_score, best_message = best_iter,
                             mod = list(xgb.naive)) 
...
By looking at the train/test results, the validation results (especially the model performance at higher Secchi estimates), and the overall r^2 of the validation, it looks like the 5 day window with the 7/1 day met data is the best performing. This is a relatively subjective decision - and the end user can absolutely make a different decision.
...
save(optimized_booster_jd_51m, file = 'data/models/optimized_xg_8_5d_71m.RData')

full_stack <- read_csv('data/upstreamRS/yojoa_corr_rrs_met_scaled_v2026-03-24.csv') %>%
  mutate(secchi = 100) %>%
  prepData(.)
...
situ_stack <- read.csv('data/in-situ/Secchi_completedataset.csv') %>% 
  #bind with updated data
  bind_rows(read.csv('data/in-situ/Secchi_updates.csv') %>% mutate(secchi = as.character(secchi))) %>% 
...
ggplot(lake_med, aes(x = date, y = secchi, color = mission, shape = mission)) + 
  ...
  theme(legend.position = c(0.8,0.8)) + 
  scale_shape_manual(values = c(19,19,19,19,1)) +
  scale_x_date(limits = c(as.Date('2018-01-01'), as.Date('2026-01-01')))+
  theme(plot.title = element_text(hjust = 0.5, face = 'bold'),
        plot.subtitle = element_text(hjust = 0.5))
...
To save teh predictions
write_csv(preds_jd %>% select(date, location, secchi, pred_secchi_jd_51m), "holdout_data_stringent.csv")
